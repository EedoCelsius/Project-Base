  ...
  <section class="space-y-6">
    <Card>
      <div class="space-y-2">
        ...
        <p class="text-sm text-surface-500">{{ hero.subtitle }}</p>
      </div>
    ...
    <div class="grid gap-6 md:grid-cols-3">
      <Card v-for="metric in metrics" :key="metric.title">
        <p class="text-sm text-surface-500">{{ metric.title }}</p>
        <div class="mt-2 text-3xl font-semibold text-surface-900">{{ metric.value }}</div>
        <p class="mt-2 text-sm text-surface-500">{{ metric.description }}</p>
    ...
    <Card>
      ...
      <div class="mt-4 overflow-hidden rounded-xl border border-surface-100">
